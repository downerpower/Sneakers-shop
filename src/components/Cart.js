...
const Cart = () => {
   ...
   return (
      <>
         {!isProceeding && !paymentCompleted && cartItems.length === 0 &&
         ...
         {!isProceeding && !paymentCompleted && <div className={styles.cartContainer}>
            ...
            {cartItems.length > 0 && <div className={styles.cartTotal}>
               ...
               <button className={styles.cartBtn} onClick={handlePaymentClick}>Proceed to payment</button>
            </div>}
         </div>}
         {!isProceeding && paymentCompleted && <p className="message--empty">Your payment is complete.</p>}
         {isProceeding && paymentCompleted && <Navigate to='/' />}
      </>
   );
}
