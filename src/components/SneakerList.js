...
const SneakerList = ({ query, selectedOption }) => {
   ...
   let fetchUrl = (pathname === '/') ? `https://example-data.draftbit.com/sneakers?${query ? `&q=${query}` : ''}${selectedOption ? `&_sort=retailPrice&_order=${selectedOption}` : ''}` : `https://example-data.draftbit.com/sneakers?gender=${pathname.slice(1)}${query ? `&q=${query}` : ''}${selectedOption ? `&_sort=retailPrice&_order=${selectedOption}` : ''}`;

   const { data, loading } = useFetch(fetchUrl);

   const uniqueData = data && [...new Map(data.map((element) => [element.title, element])).values()];

   const sneakers = uniqueData
      && uniqueData
         .filter(sneaker => sneaker.media && sneaker.retailPrice)
         .filter(sneaker => sneaker.media.imageUrl !== "https://stockx-assets.imgix.net/media/New-Product-Placeholder-Default.jpg?fit=fill&bg=FFFFFF&w=700&h=500&auto=format,compress&trim=color&q=90&dpr=2&updated_at=0")
         .filter(sneaker => sneaker.title !== 'adidas Ultraboost 19 Linen Ash Green');
   return (
      <>
         {loading && <LoopCircleLoading color='black' />}
         {!loading && sneakers && sneakers.length === 0 &&
            <p className="message--empty">Nothing mathes your search
